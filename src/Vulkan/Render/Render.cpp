...
void Render::drawFrame(uint32_t lastFrameTime) {
	...
	// Simulation submission
	{
		...
		vkResetCommandBuffer(
			commandPoolManager->getCommandBuffers()[currentFrame], 0);
	...
	// Display submission
	{
		...
		vkResetCommandBuffer(
			commandPoolManager->getQuadCommandBuffers()[currentFrame], 0);
		...
		if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR ||
			...
			swapChainManager->recreateSwapChain(window->get_GLFW_Window(),
				deviceManager->getDevice());
			descriptorManager->recreateDescriptorSets(device, instance->getQuadDescriptorSets());
		}
